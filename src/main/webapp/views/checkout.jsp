    ...
    <div class="container">
        ...
        <c:if test="${cartSize==null || cartSize==0}">
            ...
            <div class="checkout-right-basket">
                ...
                <a href="products"><span class="glyphicon glyphicon-menu-left" aria-hidden="true"></span>Continue
                    Shopping</a>
            </div>
        </c:if>
        <%--			</div>--%>
        <c:if test="${cartSize!=null && cartSize>0}">
                    ...
                    <c:forEach items="${cart}" var="order" varStatus="counter">
                        ...
                        <tr class="rem1" id="r${order.product.id}${order.size}">
                            ...
                            <td class="invert-image" id="row"><a href="item?productId=${order.product.id}">
                                <figure>
                                    <img src="image?imgName=${order.product.imageUrl}" alt=" "
                                         class="img-responsive cart-img"/>
                                ...
                                <div class="quantity">
                                    <div class="quantity-select">
                                        <div class="entry value-minus"
                                             onclick="decreaseQuan(${order.product.id},'${order.size}')"></div>
                                        <input type="hidden" id="${order.product.id}${order.size}"
                                        ...
                                        <div class="entry value"><span
                                                id="q${order.product.id}${order.size}">${order.quantity}</span></div>
                                        <div class="entry value-plus active"
                                             onclick="increaseQuan(${order.product.id},'${order.size}')"></div>
                                    </div>
                                </div>
                            </td>
                                <%--						<td class="invert" >${order.product.name}</td>--%>
                            <td class="invert">$<span
                                    id="p${order.product.id}${order.size}">${order.total / order.quantity}</span></td>
                                <%--							<td class="invert">--%>
                                <%--								<div class="rem">--%>
            ...
            <div class="checkout-left">
                <div class="checkout-left-basket">
                    ...
                    <ul>
                        <c:forEach items="${cart}" var="order" varStatus="counter">
                            <li id="rr${order.product.id}${order.size}">${order.product.name}(${order.size})
                                <i>-</i><span id="t${order.product.id}${order.size}">${order.total}</span>
                                <span>$</span></li>
                        </c:forEach>
